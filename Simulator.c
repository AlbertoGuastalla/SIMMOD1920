 ...
 *
 * Name              : Simulator.c  (Operating System Scheduling Simulator)
...
#define TIME_SLICE_STREAM 7

#define TAGGED_CUSTOMER_ID 0
#define STANDARD_NORMAL_PERCENTILE_0050 1.645

boolean firstValidationStep;					/* flag for First Step Validation Model */
boolean secondValidationStep;					/* flag for Second Step Validation Model */
...
double AVERAGE_TIME_SLICE_TIME;					/* Average time-slice time (for Second Step Validation) */
...
unsigned long CUSTOMER_IN_SWAP_IN_FOR_REGENERATION_COND;	/* Number of jobs in Swap-In station to characterize the regeneration condition */
unsigned long CUSTOMER_IN_SWAP_OUT_FOR_REGENERATION_COND;	/* Number of jobs in Swap-Out station to characterize the regeneration condition */
unsigned long CUSTOMER_IN_IO1_FOR_REGENERATION_COND;		/* Number of jobs in IO1 station to characterize the regeneration condition */
unsigned long CUSTOMER_IN_IO2_FOR_REGENERATION_COND;		/* Number of jobs in IO2 station to characterize the regeneration condition */
unsigned long SWAP_IN_CUSTOMERS;
unsigned long IO1_CUSTOMERS;
unsigned long IO2_CUSTOMERS;
...
unsigned long event_counter;        				/* Number of the events processed by the simulator*/
unsigned long cycles;						/* Number of regeneration cycles */

unsigned long nsys;						/* Number of jobs in the system */
unsigned long n_DS;						/* Number of jobs in delay station */
unsigned long n_RS;						/* Number of jobs in reserve station */
unsigned long n_SIS;						/* Number of jobs in swap-in station */
unsigned long n_CPUS;						/* Number of jobs in Cpu station */
unsigned long n_IO1S;						/* Number of jobs in IO1 station */
unsigned long n_IO2S;						/* Number of jobs in IO2 station */
unsigned long n_SOS;						/* Number of jobs in swap-out station */
unsigned long narr_DS;						/* Number of arrival at delay station */
unsigned long ncom_DS;						/* Number of departure at delay station */
unsigned long narr_RS;						/* Number of arrival at reserve station */
unsigned long ncom_RS;						/* Number of departure at reserve station */
unsigned long narr_SIS;						/* Number of arrival at swap-in station */
unsigned long ncom_SIS;						/* Number of departure at swap-in station */
unsigned long narr_CPUS;					/* Number of arrival at Cpu station */			
unsigned long ncom_CPUS;					/* Number of departure at Cpu station */
unsigned long narr_IO1S;					/* Number of arrival at IO1 station */			
unsigned long ncom_IO1S;					/* Number of departure at IO1 station */
unsigned long narr_IO2S;					/* Number of arrival at IO2 station */			
unsigned long ncom_IO2S;					/* Number of departure at IO2 station */
unsigned long narr_SOS;						/* Number of arrival at swap-out station */			
unsigned long ncom_SOS;						/* Number of departure at swap-out station */

double  	 *totalActiveTimes;				/* Total Active Times foreach job */
double  	 *totalResponseTimes;				/* Total Response Times foreach job */
unsigned long *passageSubSystemTimes;				/* Number of times that a job pass through the SubSystem (Active System) */
unsigned long *passageDelayTimes;				/* Number of times that a job pass through the Delay Station */
...
unsigned long numberOfObservationInCurrentCycle;		/* Number of observations in a regeneration cycle */
unsigned long WORKSTATIONS;					/* Number of terminals connected to the system */ 
...
int main(int argc, char* argv[]){
	...
	unsigned long validationStep = 0;
	...
	for (i = 1; i < argc; i++) {
		...
		} else if (strcmp(argv[i], "-fvs") == 0) {
			validationStep = 1;
		} else if (strcmp(argv[i], "-svs") == 0) {
			validationStep = 2;
	...
	if (validationStep == 2) {
		int ret = spawnl(P_WAIT, "MVA2.exe", "MVA2.exe", itoa (terminals, buffer, 10), NULL);	
	} else {
	...
	simulate (terminals, validationStep, verbosity);
	getchar();
    return 0;   
}
...
void initialize(int terminals, int validation, boolean verbosity){
    ...
    /* model setup */
    if (validation == 1) firstValidationStep = true;
	else firstValidationStep = false;
	
	if (validation == 2) secondValidationStep = true;
	else secondValidationStep = false;
	...
	SWAP_IN_CUSTOMERS 					= (((int)(meanValues[1] + 0.5) < 1) ? 1 : (int)(meanValues[1] + 0.5));
	IO1_CUSTOMERS						= ((int)(meanValues[3] + 0.5));
	IO2_CUSTOMERS 						= ((int)(meanValues[4] + 0.5));
	
	if (!firstValidationStep && !secondValidationStep) {
		Q33						= 0.0;
		Q34 						= 0.65;
		Q35 						= 0.25;
		Q36 						= 0.10;
		...
		CUSTOMER_IN_SWAP_IN_FOR_REGENERATION_COND 	= (MPD >= WORKSTATIONS) ? SWAP_IN_CUSTOMERS : 1;
		CUSTOMER_IN_IO1_FOR_REGENERATION_COND 		= (MPD >= WORKSTATIONS) ? IO1_CUSTOMERS : 1;
		CUSTOMER_IN_IO2_FOR_REGENERATION_COND 		= (MPD >= WORKSTATIONS) ? IO2_CUSTOMERS : 6;
	} else if (firstValidationStep) {
		Q33						= 0.9;
		Q34 						= 0.065;
		Q35 						= 0.025;
		Q36 						= 0.01;
		...
		MPD 	    					= INT_MAX;	
		CUSTOMER_IN_SWAP_OUT_FOR_REGENERATION_COND 	= 0;
		CUSTOMER_IN_SWAP_IN_FOR_REGENERATION_COND 	= SWAP_IN_CUSTOMERS;
		CUSTOMER_IN_IO1_FOR_REGENERATION_COND 		= IO1_CUSTOMERS;
		CUSTOMER_IN_IO2_FOR_REGENERATION_COND 		= IO2_CUSTOMERS;
	} else {
		Q33						= 0.0;
		Q34 						= 0.65;
		Q35 						= 0.25;
	...
	passageSubSystemTimes 			  		= (unsigned long*) calloc (WORKSTATIONS, sizeof(unsigned long));
	totalResponseTimes    			  		= (double*) calloc (WORKSTATIONS, sizeof(double));
	passageDelayTimes     			  		= (unsigned long*) calloc (WORKSTATIONS, sizeof(unsigned long)); 	
...
void arrivalCpu(nodePtr node_event){
	...
	if (secondValidationStep) {
		CPU_QUANTUM = GetNegativeExponentialRandomVariate(AVERAGE_TIME_SLICE_TIME, TIME_SLICE_STREAM);
	} 
...
void departureCpu(nodePtr node_event){
	...
	if (!firstValidationStep && !secondValidationStep) {
		...
		if (node_event->event.remained_service_time == 0.0) {
			/* Time slice is greater or equal to the service time */
			SelectStream(CPU_COMPLETION_CHOICE_STREAM);
			choice = Uniform(0.0, 1.0);
			...
			/* Time slice is lesser to the service time */
			cpuBurst = GetHyperExponentialRandomVariate(SERVICE_CPU_STREAM);
			node_event->event.service_time = cpuBurst;
			node_event->event.remained_service_time = cpuBurst;
		    node_event->event.type = ARRIVAL_CPU;
...
boolean DecideToStop () {
	return ((STANDARD_NORMAL_PERCENTILE_0050 * delta / sampleAverageActiveTime) <= 0.05);
}

void print_results(){
	double xCPU = ncom_CPUS / ObservPeriod;
	double xDS = ncom_DS / ObservPeriod;
	double xIO1 = ncom_IO1S / ObservPeriod;
	double xIO2 = ncom_IO2S / ObservPeriod;
